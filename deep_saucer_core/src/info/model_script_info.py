...
from conf.configuration import MODEL_SCRIPT, ID, PATH, UTF8, ENV_SETUP
from src.info.base_info import BaseData


class ModelScriptInfo(object):
    __data_dict = {}
    ...
    def read_conf(cls, path=''):
        try:
            with open(file=path, mode='r', encoding=UTF8) as read_file:
                ...
                if load_val and MODEL_SCRIPT in load_val:
                    for val in load_val[MODEL_SCRIPT]:
                        model = ModelScript(identifier=val[ID], path=val[PATH],
    ...
    def data(cls):
        data_list = []
        for identifier, model in cls.data_items():
            data_list.append(OrderedDict(
                {ID: model.id, PATH: model.path, ENV_SETUP: model.env_id}))

        return {MODEL_SCRIPT: data_list}
...
class ModelScript(BaseData):

    def __init__(self, path, env_id, identifier=None):
        if identifier is None:
            identifier = ModelScriptInfo.max_id() + 1
