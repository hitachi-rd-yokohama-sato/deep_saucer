...
def parse_constraint(property_path, input_names, output_names):
    ...
    with open(property_path, 'r') as f:
        ...
        return 'And({})'.format(','.join(express_list))


def _make_express_replace_names(parse_list, input_names, output_names=None):
    ret_str = ""
    ...
    if len(parse_list) == 1 and isinstance(parse_list[0], str):
        ...
        for parse in parse_list:
            ...
            if isinstance(parse, list):
                sub_str = _make_express_replace_names(parse,
                                                      input_names, output_names)
            else:
                sub_str = parse.strip('"\'')
                if sub_str in input_names:
                    sub_str = 'input_array[{}]'.format(
                        input_names.index(sub_str))
