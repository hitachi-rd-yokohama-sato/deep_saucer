...
def main(model, dataset=None, conf_path=None):
    """
    ...
    fc.coverage_rate_output()

    # MOD Function Args
    # 2, Output heat map
    if json_data["heat_map_type"] != 0:
        fc.coverage_heatmap_output()

    # MOD Function Args
    # 3, Output combination coverage
    if json_data["combination_type"] == 1:
        fc.combination_cov_output(json_data["combination_first"], json_data["combination_second"])

    # MOD Function Args
    # Dynamic instance creation
    # 4, Generate coverage increase data
    fc.test_case_generator(x_placeholders, y_placeholders, k_placeholders, x_input, y_input, k_input)
