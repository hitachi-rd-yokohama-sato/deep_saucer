...
from lib.coverage_verification import DETERMINATION_ON_ACTIVATION,\
    REPLACE_NUM, THRESHOLD, LOWER_BOUND, UPPER_BOUND, ACTIVATION_FILTER_NO


class Activation:
    def __init__(self, activation_func_name, determination_on_activation_kw,
        ...
        if activation_func_name == "sigmoid":
            self.activation = lambda x: replace(x,
...
def replace(x, determination_on_activation_kw, activation_value_add_neuron_kw):
    ...
    # If it is greater than or equal to the threshold, make it active.
    if determination_on_activation_kw[DETERMINATION_ON_ACTIVATION] == 0:
        if activation_value_add_neuron_kw[REPLACE_NUM] == 1:
            x[x >= determination_on_activation_kw[THRESHOLD]] = 1
            x[x < determination_on_activation_kw[THRESHOLD]] = 0
        else:
            x[x < determination_on_activation_kw[THRESHOLD]] = 0
    ...
    # Activate the upper μ-th value.
        ...
        activation_filter_no = determination_on_activation_kw[
        ...
        for i, x_copy in enumerate(x):
            ...
            superior_columns = np.sort(unique_x)[::-1][0:activation_filter_no]
